...
export async function append(input: AppendInput): Promise<AppendNoteResult> {
  ...
  if (input.parentLine !== undefined) {
    ...
    // appendOnly blocks root creation — must append under existing bullets
    if (config.writePermissions.appendOnly) {
      throw new Error(
        `Cannot create root bullets: appendOnly is enabled. ` +
        `Use parentLine to append under an existing tagged bullet. ` +
        `Use query_tag to find existing tagged bullets.`
      );
    }
