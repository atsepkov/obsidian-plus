...
const DEFAULT_CONFIG: MCPConfig = {
  ...
  writePermissions: {
    ...
    fullWriteFolders: ['Weekly Notes'],  // Folders allowing full document replacement
  },
  bulletConventions: {
    human: '-',
    response: '+',
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    switch (key) {
      ...
      case 'tagportalsonly':
        ...
        break;
      case 'fullwritefolders':
        config.writePermissions = {
          ...config.writePermissions || DEFAULT_CONFIG.writePermissions,
          fullWriteFolders: value.split(',').map(v => normalizeConfigVal(v.trim()) as string),
        };
        break;
      case 'humanbullet':
