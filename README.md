...
- ability to nominate special tags that are treated differently
  ...
  - tags that repeat on a period (weekly, monthly, quarterly, yearly, or any multiple)
    - the first bullet you write with the tag defines the repeating task
    - each period generates a fresh instance linked back to that definition
    - see `docs/DSL.md` → Recurring Task Tags
  - tags that suggest continuing an earlier note as you type
    - typing `- #tag some context` offers similar past bullets under that tag
    - accepting stitches back to the original, so tree of thought shows the whole thread
    - each tag defines its own continuation format, see `docs/DSL.md` → `onSuggest`
