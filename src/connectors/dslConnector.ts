...
export default class DSLConnector extends TagConnector {
    ...
    /**
     * Returns true if this tag's execution should be forwarded to another device.
     */
    private shouldDelegate(): boolean {
        return this.obsidianPlus.delegateManager?.shouldDelegate(this.tag) ?? false;
    }
    ...
    async onTrigger(task: Task, event?: { fromStatus?: string; toStatus?: string }): Promise<any> {
        ...
        if (this.shouldDelegate()) {
            return this.obsidianPlus.delegateManager.send({
                tag: this.tag, trigger: 'onTrigger',
                task, event, file: this.getTaskFile(task),
                vars: this.getDslInitialVars({ event })
            });
        }
    ...
    async onEnter(line: string, file: TFile, editor: Editor, task?: Task): Promise<DSLExecutionResult | null> {
        ...
        if (this.shouldDelegate()) {
            await this.obsidianPlus.delegateManager.send({
                tag: this.tag, trigger: 'onEnter',
                task: task ?? { path: file.path, line: 0, text: line } as any,
                file, vars: this.getDslInitialVars()
            });
            return { success: true, value: { __delegated: true }, context: {} as any };
        }
    ...
    async onStatusChange(task: Task, fromStatus: string, toStatus: string): Promise<DSLExecutionResult | null> {
        if (this.shouldDelegate()) {
            await this.obsidianPlus.delegateManager.send({
                tag: this.tag, trigger: 'onDone',
                task, event: { fromStatus, toStatus },
                file: this.getTaskFile(task),
                vars: this.getDslInitialVars({ event: { fromStatus, toStatus } })
            });
            return { success: true, value: { __delegated: true }, context: {} as any };
        }
    ...
    async onData(data: any): Promise<void> {
        ...
        if (this.shouldDelegate()) {
            ...
            await this.obsidianPlus.delegateManager.send({
                tag: this.tag, trigger: 'onData',
                task: { path: activeFile?.path ?? '', line: 0, text: '' } as any,
                file: activeFile, vars: this.getDslInitialVars({ data })
            });
            return;
        }
