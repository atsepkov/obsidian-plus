...
private async _tick() {
    const subs = this.plugin.settings.subscribe as Record<string, SubscribeEntry>;
    const now  = Date.now();
  ...
  /* ─── one network call ─────────────────────────────────────────────── */
  private async _poll(tag: string, entry: SubscribeEntry) {
