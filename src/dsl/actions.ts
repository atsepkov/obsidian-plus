...
import { App, Notice, requestUrl, TFile, normalizePath, Platform } from 'obsidian';
import type {
    DSLContext,
    ActionNode,
    ReadActionNode,
...
function getExecAsync(): (cmd: string, opts: any) => Promise<{ stdout: string; stderr: string }> {
    // Lazy-load Node.js modules — they don't exist on mobile
...
export const shellAction: ActionHandler<ShellActionNode> = async (action, context) => {
    ...
    try {
        const result = await getExecAsync()(command, {
            cwd,
