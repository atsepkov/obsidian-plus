...
export class ImageProcessor {
    ...
    async processTaskImages(task: Task): Promise<void> {
        // @ts-ignore - pageToFile exists on dataview API at runtime
        const noteFile = (this.dataview as any).pageToFile?.(task.link.path) ?? this.vault.getAbstractFileByPath(task.link.path);
        if (!noteFile) {
        ...
        // @ts-ignore - processedContent is an object with content property
        const newContent = content.replace(taskBlock.original, processedContent.content);
    ...
    private async downloadImage(url: string, noteFile: TFile): Promise<string> {
        ...
        // @ts-ignore - getConfig exists on Vault at runtime
        const attachmentPath = (this.vault as any).getConfig?.('attachmentFolderPath') || 'Attachments';
        ...
        try {
            ...
            // @ts-ignore - getResourcePath accepts string path at runtime
            return (this.vault as any).getResourcePath?.(destPath) ?? destPath;
        } catch (e) {
